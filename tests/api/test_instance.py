...
def test_instance_mesh(
    ...
    instance_mesh = instance.instance_mesh(request)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"envoy": None},
            "Deployment key fake_cluster.fake_inst not found",
            404,
        ),
        ...
        (
            "unsupported_type",
            {"envoy": None},
            "Getting mesh status for unsupported_type instances is not supported",
            405,
        ),
        ...
        (
            "flink",
            {},
            "Instance 'fake_service.fake_inst' is not configured for the mesh",
            405,
        ),
    ],
)
def test_instance_mesh_error(
    mock_validate_service_instance,
    ...
    with pytest.raises(ApiFailure) as excinfo:
        instance.instance_mesh(request)
