...
@mock.patch(
    "paasta_tools.cli.cmds.mesh_status.paasta_mesh_status_on_api_endpoint",
    autospec=True,
)
def test_report_mesh_status_for_cluster(
    mock_mesh_status_endpt, system_paasta_config,
):
    mock_mesh_status_endpt.side_effect = [
        (0, ["inst_1 line"]),
        (1, ["inst_2 line"]),
    ]

    code, output = mesh_status.report_mesh_status_for_cluster(
        cluster="fake_cluster",
        service="fake_service",
    ...
    assert output == [
        "service: fake_service",
        "cluster: fake_cluster",
        f"  instance: {PaastaColors.cyan('inst_1')}",
        "    inst_1 line",
        f"  instance: {PaastaColors.cyan('inst_2')}",
        "    inst_2 line",
    ]
